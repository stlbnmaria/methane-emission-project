...
def webapp_data_processing(data: pd.DataFrame) -> pd.DataFrame:
    ...
    data["plume_at_last_date"] = np.where(
        data["date"] == data["last_date"], np.where(data["plume"] == 1, 1, 0), 0
    )
    data["plume_at_last_date"] = data.groupby(["lat", "lon"])[
        "plume_at_last_date"
    ].transform("max")
    data["threshold_date"] = data["date"] >= (
        data["last_date"] - pd.DateOffset(months=1)
    )
    data["plume_last_month"] = np.where(
        ((data["threshold_date"]) & (data["plume"] == 1)), True, False
    )
    data["last_month"] = np.where((data["threshold_date"]), True, False)
    data = (
        data.groupby(["lat", "lon", "count", "last_date", "plume_at_last_date"])
        .apply(
            lambda x: (
                (x["plume_last_month"]).sum(),
                (x["last_month"]).sum(),
                (x["plume"]).sum(),
            )
        )
...
def table(
    left_col_color: str, right_col_color: str, selected_metadata: pd.DataFrame, i: int
) -> str:
    """
    ...
    return (
        ...
        + """;"><span style="color: #ffffff;">Last known leakage state </span></td>
            <td style="width: 250px;background-color: """
        + right_col_color
        + """;">{}</td>""".format(selected_metadata.iloc[i]["plume_at_last_date"])
        + """
            </tr>
            <tr>
            <td style="background-color: """
        ...
        + """
            ...
            <td style="background-color: """
        + left_col_color
        + """;"><span style="color: #ffffff;">Total number of leakages </span></td>
            <td style="width: 250px;background-color: """
        + right_col_color
        + """;">{}</td>""".format(selected_metadata.iloc[i]["plume_count"])
        + """
            </tr>
            <tr>
            <td style="background-color: """
        + left_col_color
        + """;"><span style="color: #ffffff;">Number of pictures last month </span></td>
            <td style="width: 250px;background-color: """
        + right_col_color
        + """;">{}</td>""".format(selected_metadata.iloc[i]["total_count_lm"])
        + """
            </tr>
            <tr>
            <td style="background-color: """
        + left_col_color
        + """;"><span style="color: #ffffff;">Number of leakages last month </span></td>
            <td style="width: 250px;background-color: """
        + right_col_color
        + """;">{}</td>""".format(selected_metadata.iloc[i]["plume_count_lm"])
        + """
            </tr>
            <tr>
            <td style="background-color: """
        + left_col_color
        + """;"><span style="color: #ffffff;">Responsible </span></td>
            <td style="width: 250px;background-color: """
        + right_col_color
        + """;">{}</td>""".format(selected_metadata.iloc[i]["Responsible"])
        + """
            </tr>
            </tbody>
            </table></center>
